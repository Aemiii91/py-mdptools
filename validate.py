...
import sys
import numpy as _np
from utils import prompt_fail, _c, lit_str
...
def validate(mdp: MDP, raise_exception: bool = True) -> tuple[bool, list[str]]:
    ...
    if not en_s_nonempty:
        for err in errors:
            buffer += [prompt_fail("forall s in S : en(s) != {}", err)]
    sum_to_one, errors = validate_sum_to_one(mdp)
    if not sum_to_one:
        for err in errors:
            buffer += [prompt_fail("forall s in S, a in en(s) : sum_(s' in S) P(s, a, s') = 1", err)]
    if len(buffer) != 0:
...
def validate_sum_to_one(mdp: MDP):
    ...
    errors = [f"{_c[_c.function, 'Dist']}({_c[_c.state, s]}, {_c[_c.action, a]}) ->"
        ...
        if (sum_a := _np.abs(sum([mdp[s, a, s_prime] for s_prime in mdp.S]))) <= 10*_np.spacing(_np.float64(1))]
    return (len(errors) == 0, errors)
