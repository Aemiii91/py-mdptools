...
def stringify(mdp: MarkovDecisionProcess) -> str:
    lines = []
    # Name and validity
    lines += [f"{_h[_h.variable, mdp.name]} -> {_h[_h.types, 'MDP']} "
              f"[{_h[_h.ok, 'Valid'] if mdp.is_valid else _h[_h.fail, 'Invalid']}]:"]
    # States and start state
    lines += [f"  {_h[_h.variable, 'S']} := {lit_str(tuple(mdp.S), _h.state)},"
              f" {_h[_h.variable, 's_init']} := {_h[_h.state, mdp.s_init]} "
             + _h[_h.comment, f"// {len(mdp.S)}"]]
    # Actions
    lines += [f"  {_h[_h.variable, 'A']} := {lit_str(tuple(mdp.A), _h.action)} "
             + _h[_h.comment, f"// {len(mdp.A)}"]]
    # Enabled transitions
    lines += [(en_s := mdp[s], f"  {_h[_h.function, 'en']}({_h[_h.state, s]}) ->"
              f" {lit_str(en_s, mdp_color_map(mdp))} " + _h[_h.comment, f"// {len(en_s)}"])[-1]
              for s in mdp.S]
    # Errors
...
def mdp_color_map(mdp: MarkovDecisionProcess) -> ColorMap:
    if _h.state == '':
        return {}
    return {
        _h.state: list(mdp.S.keys()),
        _h.action: list(mdp.A.keys())
    }
...
def format_strings(s: str, color_map: ColorMap = None) -> str:
    ...
    return re.sub(str_re, lambda x: (
        ...
        color + word + _h.reset)[-1], s)

def format_floats(s: str) -> str:
    float_re = (r"([^\w\\'])"
        r"(?:(?:(?:(0*[1-9][0-9]*)|0+)(?:\.?0+|(\.?0*[1-9][0-9]*)))|(\.[0-9]+))"
        r"([^\w\\'])")
    return re.sub(float_re, r"\1" + _h.numeral + r"\2\3\4" + _h.reset + r"\5", s)
